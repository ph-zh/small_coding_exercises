...
добавьте в код возможность менять сумму расходов за любой день через консоль. Сначала у пользователя должна быть возможность
ввести индекс траты, которую он хочет изменить, а потом её новое значение. Переменную для индекса назовите index, а переменную
для новой суммы расходов — newExpense.
 */
